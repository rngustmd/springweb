    ...
    const 게시물리스트 = 응답결과.data;
        for( let index = 0 ; index <= 게시물리스트.length-1 ; index++ ){
            ...
            html += `<tr>
                    ...
                    <td> ${ 게시물객체.count } </td> 
                    <td> <button onclick="waitingUpdate( ${ 게시물객체.no } )">수정</button> 
                         <button onclick="waitingDelete( ${ 게시물객체.no } )">삭제</button> 
                    </td> 
                    </tr>`
        } // for end 
    // 3. 출력 , <마크업> inner </마크업>   
...
async function waitingSave(){ // 동기화함수 async
    ...
    const response = await axios.post( `/waiting/save?phone=${ phone }&count=${ count }` );
    // 3. 결과출력 
...
// [3] 수정 (번호 no 기준 인원수 수정)
async function waitingUpdate( no ){
    // 1. 수정할 내용 입력받기 prompt
    const count = prompt('수정할 인원수: ');
    // 2. 수정처리 : axios 이용하여 백엔드에게 수정 요청/응답
    const response = await axios.put( `/waiting/update?no=${ no }&count=${ count }`);
    // 3. 결과
    if( response.data == true ){
        alert('수정 성공');
        waitingFindAll();
